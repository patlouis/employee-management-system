                  ...
                  {[
                    ...
                    <th
                      ...
                      className={`px-4 py-2 text-left font-bold ${
                        h.sortable === false ? "" : "cursor-pointer hover:bg-gray-200 transition select-none"
                      }`}
              ...
              <tbody>
                {positions.map((p) => (
                  <tr key={p.position_id} className="border-b last:border-b-0 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2">{p.created_at ? dayjs(p.created_at).format("YYYY-MM-DD HH:mm") : "—"}</td>
                    <td className="px-4 py-2">{p.updated_at ? dayjs(p.updated_at).format("YYYY-MM-DD HH:mm") : "—"}</td>
                    <td className="px-4 py-2 flex gap-2">
                      <button onClick={() => setEditData({ ...p })} className="p-1 rounded hover:bg-gray-100 cursor-pointer">
                        <Edit2 className="w-4 h-4 text-blue-600" />
                      </button>
                      <button onClick={() => handleDelete(p.position_id)} className="p-1 rounded hover:bg-gray-100 cursor-pointer">
...
// Optional reusable Modal component
function Modal({ title, onClose, onSubmit, departments, data, setData }) {
  return (
      ...
      <div className="bg-white p-6 rounded-xl w-full max-w-md shadow-lg">
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          <input
            ...
            name="title"
            value={data?.title || ""}
            onChange={(e) => setData && setData({ ...data, title: e.target.value })}
          ...
          <input
            ...
            onChange={(e) => setData && setData({ ...data, description: e.target.value })}
            className="w-full border rounded px-3 py-2 focus:outline-none focus:ring focus:ring-blue-200"
          />
          <select
            name="department_id"
            value={data?.department_id || ""}
            onChange={(e) => setData && setData({ ...data, department_id: e.target.value })}
            className="w-full border rounded px-3 py-2 focus:outline-none focus:ring focus:ring-blue-200"
            required
          >
          ...
          <div className="flex justify-end gap-2 mt-2">
            <button type="button" onClick={onClose} className="px-4 py-2 rounded bg-gray-200 hover:bg-gray-300 cursor-pointer">
              Cancel
            </button>
